...
class LabubuGenerator {
    constructor() {
        ...
        // Accessory images (PNG hex strings - to be filled in later)
        this.accessories = [
            /*pink glasses*/"",
            /*horny*/""
        ];
    }
    ...
    /**
     * Overlay accessory on base image
     * @param {HTMLCanvasElement} baseCanvas - Base image canvas
     * @param {string} accessoryHex - Accessory hex string
     * @returns {Promise<HTMLCanvasElement>} - Combined image canvas
     */
    async overlayAccessory(baseCanvas, accessoryHex) {
        const accessoryImg = await this.loadImageFromHex(accessoryHex);
        ...
        outCtx.drawImage(accessoryImg, 0, 0);
        
        return outCanvas;
    }

    /**
     * Generate a random number generator with seed
    ...
    async labubuMaker(seed) {
        ...
        // Load base image
        const baseImg = await this.loadImageFromHex(baseHex);
        ...
        // Generate random hue for recoloring
        const newHue = this.randomFloat(rng, 0, 360);
        ...
        for (let i = 0; i < data.length; i += 4) {
            ...
            const minSat = 0.6;
            const sat = hsl.s < 0.05 ? minSat : hsl.s;
            
            // Convert back to RGB with new hue
            const newRgb = this.hslToRgb(newHue, sat, hsl.l);
        ...
        // Randomly decide if we should add an accessory (30% chance)
        const shouldAddAccessory = this.randomBool(rng, 0.3);
        
        let finalCanvas = canvas;
        
        if (shouldAddAccessory && this.accessories.length > 0) {
            // Choose random accessory
            const accessoryIdx = this.randomInt(rng, 0, this.accessories.length);
            const accessoryHex = this.accessories[accessoryIdx];
            
            if (accessoryHex) {
                // Overlay accessory
                finalCanvas = await this.overlayAccessory(canvas, accessoryHex);
            }
        }
    ...
    /**
     * Set accessory images
     * @param {Array<string>} accessories - Array of hex strings
     */
...
//     "sleepy_hex_string_here"
// ]);
// 
// generator.setAccessories([
//     "pink_glasses_hex_string_here",
